...
export default function CollectionMintPage() {
  ...
  useEffect(() => {
    const loadCollectionData = async () => {
      if (!collectionId) {
        setError('No collection ID provided in the URL.');
      ...
      try {
        ...
        if (data.success) {
          ...
          setNftName(data.data.name); // Pre-fill NFT name from collection
          setNftDescription(data.data.description); // Pre-fill NFT description
        } else {
    ...
    if (collectionId) {
      ...
      setIsLoadingCollection(false); // No collection ID, so not loading
    }
  }, [collectionId]);
  ...
  // Check mint status when wallet connects or collection data loads
  useEffect(() => {
    if (connected && publicKey && collectionData) {
      checkMintStatus(publicKey.toString());
    }
  }, [connected, publicKey, collectionData]);
  ...
  const handleMint = async () => {
    ...
    try {
      const payload = {
        ...
        imageUrl: collectionData.imageUrl, // Use collection's fixed image
        collectionId: collectionData.id, // Pass collection ID to backend
      };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 flex items-center justify-center">
        <div className="bg-red-500/20 border border-red-500/50 rounded-lg p-6 text-red-200 text-center">
          ...
          <p>{error}</p>
          <p className="mt-4 text-sm">Please ensure the URL is correct and the collection exists.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 flex items-center justify-center">
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 text-white text-center">
          <h2 className="text-2xl font-bold mb-4">No Collection Selected</h2>
          <p>Please navigate to a specific collection mint page (e.g., /mint/collection-name?id=YOUR_COLLECTION_ID).</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-16">
            ...
            <p className="text-gray-300 text-xl mb-4">
              Collection: <span className="font-mono text-blue-400 bg-blue-500/20 px-3 py-1 rounded-lg">{collectionName}</span>
            </p>
            <p className="text-gray-300 text-lg mb-4">
              ID: <span className="font-mono text-blue-400 bg-blue-500/20 px-3 py-1 rounded-lg">{collectionId}</span>
            </p>
